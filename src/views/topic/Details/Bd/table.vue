<template>
	<div class="container">
		...
		<div class="table">
			<div v-for="item in tableData" :key="item.blbcId" :class="{ active: active === item.blbcId }">
				<div class="title">
					<h3 @click="handleCheck(item)">{{ item.troopsName }}</h3>
				</div>
				<!-- <div class="content">
					<div class="right">
						<p>{{ item.feature }}</p>
...
<script>
import { listSubjectBlbc } from "@/api/topic/resource.js"
...
export default {
	...
	methods: {
		...
		async getTableData() {
			...
			const { rows, total } = await listSubjectBlbc(query)
			if (rows && rows.length > 0 && !this.active) {
				this.active = rows[0].blbcId
			}
